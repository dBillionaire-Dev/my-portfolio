...
//                     </SafeSyntaxHighlighter> as any as React.ReactNode
//                   ) : (
...
// 2. Dynamic Import (Casted to any to avoid the TS2786 'refs' error)
const SafeSyntaxHighlighter = lazy(() => 
  import('react-syntax-highlighter').then(module => ({ default: module.Prism as any }))
);
            ...
            <ReactMarkdown
              ...
              components={{
                code({ node, inline, className, children, ...props }: any) {
                  const match = /language-(\w+)/.exec(className || "");
                  return !inline && match ? (
                    // 3. Wrap with Suspense to handle the loading state of the highlighter
